...
config_graphsDirectory = config_graphsDirectory + 'evolution/'
...
def modules_in_time(param, value):
	"""
	...
	for module in result:
		...
		start = first_commit("log","module_id = " + str(module[1]))
		if start:
			year = int(start[0:4])
			month = int(start[5:7])
			day = int(start[8:10])
			...
			#	print str(time.gmtime(point)[0]) + " " + str(time.gmtime(point)[1]) + " " + str(time.gmtime(point)[2])
				count = querySQL(param, 'log', "date_log < '" + str(time.gmtime(point)[0]) + "-" + str(time.gmtime(point)[1]) + "-" + str(time.gmtime(point)[2]) + "' AND module_id='" +  str(module[1]) + "'")
...
def commiters_in_time(param, value):
	"""
	if the given parameter is:
	SUM(commits)            --> number of commits
	SUM(plus - minus)       --> number of Lines Of Code	
	"""
	result = querySQL('commiter_id, commiter', 'cvsanal_commiters_id')

	for commiter in result:
		print "Creating graph for " + commiter[1]
		start = first_commit_commiter(commiter[0])
		if start:
			year = int(start[0:4])
			month = int(start[5:7])
			day = int(start[8:10])
...
#				print "date < '" + str(time.gmtime(point)[0]) + "-" + str(time.gmtime(point)[1]) + "-" + str(time.gmtime(point)[2]) + "'"
				count = querySQL(param, 'cvsanal_commiters_date', "date < '" + str(time.gmtime(point)[0]) + "-" + str(time.gmtime(point)[1]) + "-" + str(time.gmtime(point)[2]) + "' AND commiter_id='" + commiter[0] + "'")
...
def repository_in_time():
	"""
	"""

	# Looking for first commits
	firstList = uniqueresult2list(querySQL('MIN(first_commit) AS first',
	...
	lastList = uniqueresult2list(querySQL('MAX(last_commit) AS last',
					       'cvsanal_temp_modules',
					       '',
					       'last',
					       'module'))
...
def graph_evolution():
	...
	modules_in_time('COUNT(*)', 'commits')
	modules_in_time('COUNT(DISTINCT(file_id))', 'files')
	modules_in_time('COUNT(DISTINCT(commiter_id))', 'commiters')
	modules_in_time('SUM(plus - minus)', 'LOCs')

	if config_commiters_in_time:
		print "Creating several evolution graphs for commiters"
		
		commiters_in_time('SUM(commits)','commits')
		commiters_in_time('SUM(files)','files')
		commiters_in_time('SUM(plus)-SUM(minus)','LOCs')
		commiters_in_time('COUNT(DISTINCT(modules))','modules')
